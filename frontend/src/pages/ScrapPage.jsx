                ...
                <div style={{ maxHeight: 520, overflowY: 'auto' }}>
                  {mosLoading ? (
                    ...
                    return (
                      <div
                        key={mo.id}
                        onClick={() => setSelectedMOs(isSelected ? [] : [mo])}
                        style={{
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 20 }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center', background: '#fff', border: '1px solid #e5e7eb', padding: '6px 12px', borderRadius: 8, boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
                ...
                <input type="date" value={filterDate} onChange={e => { setFilterDate(e.target.value); setFilterStart(''); setFilterEnd(''); }} style={{ border: 'none', background: '#f9fafb', padding: '4px 8px', borderRadius: 6, fontSize: 13, color: '#4b5563', outline: 'none' }} title="Specific Date" />
                <span style={{ fontSize: 13, color: '#9ca3af', margin: '0 4px' }}>OR</span>
                <input type="date" value={filterStart} onChange={e => { setFilterStart(e.target.value); setFilterDate(''); }} style={{ border: 'none', background: '#f9fafb', padding: '4px 8px', borderRadius: 6, fontSize: 13, color: '#4b5563', outline: 'none' }} title="Start Date" />
                <span style={{ fontSize: 12, color: '#9ca3af' }}>to</span>
                <input type="date" value={filterEnd} onChange={e => { setFilterEnd(e.target.value); setFilterDate(''); }} style={{ border: 'none', background: '#f9fafb', padding: '4px 8px', borderRadius: 6, fontSize: 13, color: '#4b5563', outline: 'none' }} title="End Date" />
            ...
            <div className="card">
              <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                  {selectedScrapIds.length > 0 && (
                    <button className="btn btn-sm" onClick={handleBulkDeleteScrap} style={{ background: '#fca5a5', color: '#7f1d1d', border: 'none' }}>
                      Delete Selected ({selectedScrapIds.length})
                    </button>
                  )}
                  <div style={{ position: 'relative', width: 180 }}>
                    <GlassIcon name="search" size={12} color="#9ca3af" style={{ position: 'absolute', left: 8, top: '50%', transform: 'translateY(-50%)' }} />
                    <input placeholder="Search MO..." value={scrapSearch} onChange={e => setScrapSearch(e.target.value)} style={{ paddingLeft: 26, fontSize: 12, width: '100%' }} />
                  </div>
              ...
              {scrLoading ? (
                ...
                <div className="empty-state"><p>No scrap records found.</p></div>
              ) : (
          ...
          <div className="modal-overlay" onClick={() => setEntryModal(false)}>
            <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 480 }}>
              ...
              <div style={{ padding: '20px 24px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                  <div className="form-group">
                    ...
                    <input type="number" min="1" value={rejQty} onChange={e => setRejQty(e.target.value)} />
                  </div>
                  <div className="form-group">
                    <label style={{ color: '#16a34a', fontWeight: 700 }}>Receive (RT)</label>
              ...
              <div className="modal-footer">
                ...
                <button className="btn btn-primary" onClick={submitEntry} disabled={submitting}>Submit Scrap</button>
              </div>
            </div>
          </div>
        </ModalPortal>
            ...
            <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 480 }}>
              ...
              <div style={{ padding: '20px 24px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
                  <div className="form-group">
                    <label style={{ color: '#dc2626', fontWeight: 700 }}>Reject (RJ)</label>
                    <input type="number" min="1" value={editRejQty} onChange={e => setEditRejQty(e.target.value)} />
                  </div>
                  <div className="form-group">
                    <label style={{ color: '#16a34a', fontWeight: 700 }}>Receive (RT)</label>
